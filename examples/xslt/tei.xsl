...
AAA<xsl:apply-templates/>B
	...
	<xsl:template name="sense_vote">
		<xsl:if test="$allow_voting = 'true'">
			<xsl:element name="a"><xsl:attribute name="style"><xsl:text>font-size: small; color: blue; text-decoration: none;</xsl:text></xsl:attribute><xsl:attribute name="href"><xsl:value-of select="concat('submitvote.jsp?type=sense&amp;lexquery=',$lexquery,'&amp;doc=',$source_id,'&amp;subquery=',$subquery,'&amp;form=',$form,'&amp;which=',$which,'&amp;sense_id=',@id,'&amp;lang=',$voting_lang)"/></xsl:attribute>
	      [select]
	  </xsl:element>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="figure[@n!='' and not(descendant::p)]">
		<xsl:variable name="align">
			<xsl:call-template name="figalign"/>
		</xsl:variable>
		<p>
			<table width="120" align="{$align}">
				<tr>
					<td>
						<a target="figure" href="http://www.perseus.tufts.edu/cgi-bin/image?lookup={@n}">
							<img border="1" src="http://www.perseus.tufts.edu/cgi-bin/image?lookup={@n}&amp;thumb=1"/>
						</a>
					</td>
				</tr>
				<tr>
	...
	<xsl:template match="figure[@n!='' and (descendant::p)]">
		<xsl:choose>
	...
	<xsl:template match="figure[not(descendant::p)]">
		...
		<p>
			<table width="120" align="{$align}">
				<tr>
					<td>
						<a target="figure" href="http://www.perseus.tufts.edu/cgi-bin/image?lookup={$imID}">
							<img border="1" src="http://www.perseus.tufts.edu/cgi-bin/image?lookup={$imID}&amp;thumb=1"/>
						</a>
					</td>
				</tr>
				<tr>
	...
	<xsl:template match="figure[descendant::p]">
		<xsl:variable name="fID">
			<xsl:value-of select="substring-after(./@id,'fig.')"/>
		</xsl:variable>
		<xsl:variable name="figID">
	...
	<xsl:template match="note[@place='inset']">
		<div style="float:right"><xsl:apply-templates/>&#xA0;
    </div>
	</xsl:template>
	<xsl:template match="note//note"><xsl:apply-templates/>&#xA0;
  </xsl:template>
  ...
  -->
	...
	<xsl:template match="pb">
		...
		[<a target="figure" href="http://repository01.lib.tufts.edu:8080/fedora/get/tufts:perseus.image.2001.05.0007.{$figID}/bdef:TuftsImage/getMediumRes/"><xsl:value-of select="./@n"/></a>]   		
	  </xsl:when>
			<xsl:when test="@id and starts-with($document_id, 'Perseus:text:2001.05.0044')">
		[<a target="figure" href="http://repository01.lib.tufts.edu:8080/fedora/get/tufts:perseus.image.2001.05.0044.{$figID}/bdef:TuftsImage/getMediumRes/"><xsl:value-of select="./@n"/></a>]   		
	  </xsl:when>
			<xsl:when test="@id and starts-with($document_id, 'Perseus:text:2001.05')">
		[<a target="figure" href="http://www.perseus.tufts.edu/cgi-bin/image?lookup={$imID}"><xsl:value-of select="./@n"/></a>]   		
	  </xsl:when>
			<xsl:when test="@id">
...
-->
	...
	<xsl:template match="winner|race|date"><xsl:apply-templates/>,
    <xsl:text> </xsl:text></xsl:template>
	<xsl:template match="strophe">
		<p>
      [<xsl:value-of select="@name"/>.]<br/><xsl:apply-templates/></p>
	</xsl:template>
